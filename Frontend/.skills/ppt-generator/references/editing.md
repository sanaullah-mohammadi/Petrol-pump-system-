...
Analyze the existing slides:

python -m markitdown template.pptx
Review the markitdown output to understand placeholder text and slide structure.

Plan slide mapping: Choose a template slide for each content section.
...
Inventory tables and charts (do this before editing text):
...
Build the presentation structure (do this yourself, not with subagents):
...
Edit text content — if subagents are available, use them here (slides are independent XML files, parallelizable):
...
Replace table data (if tables present — see Table Data Replacement below)

Replace chart data (if charts present — see Chart Data Replacement below)
...
Read the chart XML
Replace <c:v> values inside <c:cat> (labels) and <c:val> (numbers)
If changing the number of data points, update <c:ptCount val="N"/> and adjust idx attributes
