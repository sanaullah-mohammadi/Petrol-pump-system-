...
Fixed card with known line count Manually calculate h 2. Text Overflowing Background Shape
...
x: card.x + PADDING,
y: card.y + PADDING,
w: card.w - PADDING _ 2,
h: card.h - PADDING _ 2, // ← Must be derived from card.h, not hard-coded
fontSize: 11,
...
// Formula: h = lineCount _ lineHeight + 0.1
const h = 3 _ 0.26 + 0.1; // 14pt, 3 lines
...
x: node.x - DOT_R, y: AXIS_Y - DOT_R, w: DOT_R _ 2, h: DOT_R _ 2,
fill: { color: "2563EB" }, line: { color: "2563EB" },
});

// Stem: starts from dot edge, end = start + STEM_H
const stemStart = AXIS_Y + dir _ DOT_R;
const stemEnd = stemStart + dir _ STEM_H;
slide.addShape(pres.shapes.LINE, {
...
for (let i = 0; i < N - 1; i++) {
const lineX1 = START_X + i _ step + CIRCLE_R; // Start from circle edge
const lineX2 = START_X + (i + 1) _ step - CIRCLE_R; // End at next circle edge
...
for (let i = 0; i < N; i++) {
const cx = START_X + i _ step;
slide.addShape(pres.shapes.OVAL, {
x: cx - CIRCLE_R, y: ROW_Y - CIRCLE_R,
w: CIRCLE_R _ 2, h: CIRCLE_R _ 2,
fill: { color: "1E3A5F" }, line: { color: "F4A300", width: 3 },
});
slide.addText(String(i + 1), {
x: cx - CIRCLE_R, y: ROW_Y - CIRCLE_R,
w: CIRCLE_R _ 2, h: CIRCLE_R \* 2,
