PptxGenJS Reference
This file is an API quick reference. For layout pitfalls, overlap/overflow fixes, and chart bugs, see pitfalls.md — required reading before writing any JavaScript.
...
For more visual/layout issues see pitfalls.md.

Colors must not have # prefix — "FF0000" ✅ / "#FF0000" ❌ (corrupts file)
Never encode opacity in 8-digit color values — use the opacity property instead
Use bullet: true for bullets — not Unicode • (causes double bullets)
Use breakLine: true to separate array items for multi-line text
Use paraSpaceAfter for bullet spacing — lineSpacing produces too much space
Use a fresh instance per presentation — do not reuse the pptxgen() object
Never reuse options objects across calls — PptxGenJS mutates objects in place (e.g. converts shadow values to EMU). Sharing the same object across calls corrupts the second shape.
...
ROUNDED_RECTANGLE must not be stacked with accent borders — the rectangle strip cannot cover rounded corners; use RECTANGLE instead
