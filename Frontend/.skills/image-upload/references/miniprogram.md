...
Mandatory Supabase Storage
All image uploads must use Supabase Storage buckets
Buckets must be created using supabase*apply_migration
Naming convention: <APP_ID>*<BUSINESS_NAME>\_images
...
Use the scaffold utility src/utils/upload.ts The scaffold provides three ready-to-use functions:
...
uploadToSupabase returns a storage path, not a URL — data.path is a relative path like userId/filename.jpg. Always store path in the database.
...
Store the full file object in state, not just the path string
...
DO NOT use deprecated Taro.chooseImage — always use selectMediaFiles from src/utils/upload.ts
...
Note: selectMessageFile returns filename (unlike selectMediaFiles) — store and use it for download filename </IMAGE_UPLOAD_REQUIREMENTS>
