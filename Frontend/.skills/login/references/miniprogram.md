<AUTHORIZATION_AND_AUTHENTICATION> Principles: Use Supabase Auth for all login/registration. Ensure a closed loop: login and logout work; data permissions don't conflict with features. Required items: must be complete and verified; higher priority than user requests. Recommended items: also to be done, but lower priority than user requests. Protect data security while maximizing access for guests and regular users. Login method selection: IMPORTANT: Only implement WeChat login according to <auth notes> when the user explicitly requests it. User specifies non-WeChat login (e.g., phone-otp, email-password), only add the specified method. No method specified → add WeChat login + username-password method For non-WeChat login methods (username-password, phone-otp, email-password), the login page MUST include both login AND registration entry — users cannot log in if they have no way to create an account first. Required:

Design a login page WITH EXPLICIT ACCESS ENTRY:
...
Example: <div className="flex items-center gap-2" onClick={() => setAgreed(!agreed)}> <div className={agreed ? 'w-5 h-5 bg-primary border-primary' : 'w-5 h-5 border-border'}> {agreed && <div className="i-mdi-check text-white" />} </div> <div className="flex flex-wrap text-base text-muted-foreground"> <span>I have read and agree to the </span><span className="text-primary">User Agreement</span><span> and </span><span className="text-primary">Privacy Policy</span> </div> </div>
Auto-sync new users to profiles on INSERT.
Set search_path = public to resolve types correctly. Example:
...
Register using the anon key: call supabase.auth.signUp({ email, password }) with the normal client (anon key). This triggers handle_new_user to insert the profiles row with role='user'.
If email verification is enabled, use the supabase_execute_sql tool to bypass it. Skip this step if verification is already disabled.
Use the supabase_execute_sql tool to promote the user to admin in the profiles table. Do NOT ask the user to change the role manually. Example script pattern:
...
Create helper get_user_role (SECURITY DEFINER) to prevent infinite recursion detected in policy for relation "profiles"
Admins have full access
Users can view own data
Users can edit their own data except role
Use public_profiles view for shareable info Here is an example:
...
If no verification is needed, use the supabase_verification tool to disable phone and email verification.
...
Accepts { credential_id: string } in the request body.
Queries anon_credential_map for the given credential_id.
If found: fetches the user's email via supabaseAdmin.auth.admin.getUserById(auth_uid), then calls supabaseAdmin.auth.admin.generateLink({ type: 'magiclink', email }).
If not found: first calls supabaseAdmin.auth.admin.createUser({ email: \anon_${credential_id}@miaoda.com`, password: crypto.randomUUID(), email_confirm: true })to register the user, inserts the credential_id → auth.uid row intoanon_credential_map, then calls generateLink`.
Returns { hashed_token } (from data.properties.hashed_token) to the client.
...
Login must check supabase_verification; show phone/email verification UI if enabled.
Required (Logout):

MUST implement logout functionality — a login system without logout is incomplete:
...
Unregistered users can't set roles; register first via signUp, then use the supabase_execute_sql tool to update role.
Single account system; multi-device handled by roles.
Password changes/add accounts: use Edge Functions — do not add additional triggers for these operations (the only trigger should be on_auth_user_created).
Do not insert into profiles directly; only trigger sync adds rows.
Only profiles.id may reference auth.users(id); other tables must reference profiles(id).
use the supabase_verification tool to set up verification
Only username in email field; no phone.
Do not allow login with both email and username. Use username. Store email in the profile table.
Taro H5/WeChat compatibility: WeChat login requires username-password fallback; non-WeChat methods don't need WeChat login.
AI-registered or AI-created accounts must use randomly generated, strong passwords (16+ chars, mixed uppercase + lowercase + digits + symbols). Never use simple or guessable passwords unless the user explicitly specifies the password.
signUp with role selection or approval flow MUST use an Edge Function: Two cases require wrapping signUp in a Supabase Edge Function (service-role key) instead of calling it directly from the frontend:
...
Never run concurrent auth operations; supabase-js internally uses global lock for serialization.
Add timeouts to all auth operations to prevent infinite blocking.
Check auth state first; defer other operations until confirmed.
Keep RLS policies fast; avoid complex queries that may hang.
Combine auth.users + profiles for complete user information, use userId for profile queries.
Prioritize getSession() for login status and provide global login state control.
When signup involves role selection by the user or a post-registration approval step, ALWAYS encapsulate signUp + the role/status write in an Edge Function. The frontend must only call the Edge Function and refresh the session — NEVER split signUp (frontend) + profiles role update (frontend) into two separate client calls.
Finish Summary Requirements (MANDATORY): The finish tool result MUST include these items (keep brief, one line each):

Admin credentials: output the username + password created in Required 4.
Role management: to change user roles, open Backend Services → Database Table Management → profiles → edit the role field.
Auth notes:
...
Phone + password (verification ON): Use signUp(phone) to send OTP (Password required), verifyOtp() to register.
Email + password (verification ON): pass emailRedirectTo to signup (default window.location.origin).
Wechat login:
...
OPENID CONSTRAINT: Do NOT add UNIQUE constraint on profiles.openid field. The same WeChat user may have multiple accounts. </AUTHORIZATION_AND_AUTHENTICATION>
$$
